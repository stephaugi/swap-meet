...
def test_swap_best_by_category_reordered():
    ...
    for item in my_expected_inventory:
        assert item in tai.inventory

    for item in their_expected_inventory:
...
def test_swap_best_by_category_no_match_is_false():
    ...
    assert len(tai.inventory) == 3
    assert len(jesse.inventory) == 3
    assert tai.inventory == my_expected_inventory
    assert jesse.inventory == their_expected_inventory
...
def test_swap_best_by_category_no_other_match_is_false():
    ...
    my_expected_inventory = [item_c, item_b, item_a]
    their_expected_inventory = [item_f, item_e, item_d]
